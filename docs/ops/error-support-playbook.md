...
| **Appeal workflow** — appeal stuck in `UNDER_APPEAL` well past its deadline | The appeal vote never got finalized. `finalize_appeal` is permissionless, so this is a keeper problem, not a contract one. Admin can force-finalize via `POST /admin/claims/:id/finalize-appeal`. Full diagnosis/recovery: **[appeal-incident-runbook.md](./appeal-incident-runbook.md)**. | **Backend** (keeper); **Contract** only if a manual `finalize_appeal` also fails | Claim id, `appeal_deadline_ledger`, current ledger |
